    ...
    StakedAvaxStorage
{
    ...
    /**
     * @notice Backfill previously not set exchange rates
     * @param timestamps An array of timestamps
     * @param exchangeRates An array of exchange rates
     */
    function setHistoricalExchangeRatesByTimestamp(uint[] memory timestamps, uint[] memory exchangeRates) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "DEFAULT_ADMIN_ROLE");
        require(timestamps.length == exchangeRates.length, "INVALID_ARRAY_LENGTH");

        for (uint i = 0; i < timestamps.length; i = i.add(1)) {
            require(historicalExchangeRatesByTimestamp[timestamps[i]] == 0, "EXCHANGE_RATE_ALREADY_EXISTS");
            require(exchangeRates[i] >= 1e18, "INVALID_EXCHANGE_RATE");

            historicalExchangeRatesByTimestamp[timestamps[i]] = exchangeRates[i];
        }
    }
